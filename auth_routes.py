...
async def Login(login_schemas : OAuth2PasswordRequestForm = Depends(), session: Session = Depends(pegar_sessao_db)):
    #o Login precisa de email e senha para verificar a validade do úsuario
    #Para ocorrer a verificação iremos criptografar toda senha inserida pelo
    #úsuario e comparar com a senha  inserida no banco de dados
    #precisamos de um loginschemas que vai conter as informações que precisam paa o login
    usuario = autentificar_usuario(login_schemas.username, login_schemas.password, session)
    if not usuario:
        raise HTTPException(status_code = 400, detail="Úsuario não encontrado ou credenciais invalída")
    else:
        #criar um token para o úsuario utilizar nas outras partes do istema e poder continuar com o acesso sem precisar sair.
        access_token = criar_token(login_schemas.username)
        refresh_token = criar_token(login_schemas.username, timedelta(days = 7))
        return {"access_token":access_token,\
                "token_type":"Bearer"}
